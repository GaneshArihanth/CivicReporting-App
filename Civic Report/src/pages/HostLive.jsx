...
const HostLive = () => {
  ...
  // Keep per-viewer peer connections and unsubscribers
  const peersRef = useRef(new Map()); // peerId -> { pc, unsubAnswer, unsubViewerCandidates }
  ...
  const hostRespondToViewer = async (peerDoc) => {
    ...
    // Host ICE candidates -> write to candidatesHost
    ...
    // Watch viewer candidates and answer
    ...
    const unsubViewerCandidates = onSnapshot(peerRef, (snap) => {
      ...
      (data?.candidatesViewer || []).forEach(async (c) => {
        try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch (e) { /* ignore dup */ }
      });
    ...
    await setDoc(peerRef, { offer: offer.toJSON ? offer.toJSON() : offer, createdAt: new Date().toISOString() }, { merge: true });
  ...
  const startLive = async () => {
    try {
      ...
      try {
        ...
        toast.error('Cannot access camera/mic. Please grant permissions and try again.');
        throw err; // bail out of startLive
      }
      
      // Now that we have camera access, create the session
      ...
      // Start recording for post-live handoff
      startRecording();

      // Watch viewer count and comments
      ...
      // Watch peers collection for new viewers
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      ...
      {sessionId && (
        <div className="mb-3">
          <div className="text-sm text-gray-700">Share link with viewers:</div>
          <div className="p-2 bg-gray-100 rounded text-xs break-all">{shareUrl}</div>
        </div>
      )}
